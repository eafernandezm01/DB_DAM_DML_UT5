...
select * from animal where id_animal=1 order by nombre, raza desc;
...
select * from animal where fecha_defuncion is not null;
...
select cp, NIF, edad from persona where cp is not null;
...
select nif, concat(nombre, ' ', apellido1) 'nombre_persona', edad from persona where edad < 30 order by nombre_persona;
select nif, concat_ws(' ', nombre, apellido1) 'nombre_persona', edad from persona where edad < 30 order by nombre_persona;
...
select nif, concat(nombre, ' ',apellido1, ' ', apellido2) 'nombre_persona' from persona order by nombre_persona;
select nif, concat_ws(' ', nombre, apellido1, apellido2) 'nombre_persona' from persona order by nombre_persona;

-- Arregla la consulta para que no haya null en nombre de persona
select nif, concat(nombre, ' ',apellido1, ' ', ifnull(apellido2, '')) 'nombre_persona' from persona order by nombre_persona;
select nif, concat_ws(' ', nombre, apellido1, ifnull(apellido2, '')) 'nombre_persona' from persona order by nombre_persona;
...
select upper(nombre), edad from animal where edad in (2, 3);
select upper(nombre), edad from animal where edad = 2 or edad = 3;
