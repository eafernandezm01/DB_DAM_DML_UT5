...
*/
select d.id, d.nombre, CONCAT_WS(' ',e.nombre, e.ape1, e.ape2) nombre_completo
from departamento d
...
SELECT d.id, d.nombre, CONCAT_WS(' ', e.nombre, e.ape1, e.ape2) AS nombre_completo
FROM departamento d
JOIN empleado e ON d.id = e.departamento_id
WHERE d.edificio = 'Edificio A'
ORDER BY d.nombre ASC, nombre_completo ASC;
...
FROM departamento d
JOIN empleado e ON d.id = e.departamento_id
WHERE d.nombre = 'IT' or d.nombre = 'MARKETING'
ORDER BY d.nombre ASC, nombre_completo ASC;
...
FROM departamento d
JOIN empleado e ON d.id = e.departamento_id
WHERE d.nombre in ('IT', 'MARKETING')
ORDER BY d.nombre ASC, nombre_completo ASC;
...
FROM departamento d
JOIN empleado e ON d.id = e.departamento_id
WHERE d.nombre in ('IT', 'MARKETING') or d.edificio = 'Edificio E'
ORDER BY d.nombre ASC, nombre_completo ASC;
 ...
 */
 select concat_ws(' ', ex.nombre, ex.ape1, ex.ape2) as nombre_emp, 
 ex.fecha_subcontratacion, d.nombre as 'nombre_Dpto',
 em.nombre nombreSubcontrata
 from empleado_externo ex
 join departamento d ON ex.departamento_id = d.id
 join empresa em ON ex.cif_subcontrata = em.CIF
 order by nombreSubcontrata, ex.fecha_subcontratacion desc;
...
select d.nombre, concat_ws(' ',e.nombre,e.ape1,e.ape2) nombre_completo, v.total, v.fecha_venta
from departamento d
...
*/
SELECT e.id, d.nombre nombre_dpto, CONCAT_WS(' ', e.nombre, e.ape1, e.ape2) nombre_empleado, e.edad, e.comision, v.total, v.fecha_venta
FROM departamento d
JOIN empleado e ON d.id = e.departamento_id
JOIN venta v ON e.id = v.empleado_id
WHERE (e.edad between 30 and 55) or e.comision is null
ORDER BY nombre_empleado asc, v.fecha_venta desc;



-- Consulta para obtener el nombre del departamento y la suma de total de ventas realizadas por los empleados.
SELECT d.nombre,  SUM(v.total) AS total_ventas
FROM departamento d
JOIN empleado e ON d.id = e.departamento_id
JOIN venta v ON e.id = v.empleado_id
GROUP BY d.nombre;
...
JOIN empleado e ON d.id = e.departamento_id
JOIN venta v ON e.id = v.empleado_id
WHERE (e.edad between 22 and 30) or comision is null 
GROUP BY d.nombre
...
JOIN empleado e ON d.id = e.departamento_id
JOIN venta v ON e.id = v.empleado_id
WHERE ((e.edad between 22 and 30) or comision is null) and d.nombre IN ('Ventas', 'Marketing')
GROUP BY d.nombre;

-- Obten el nombre completo del jefe del empleado Elena Pérez Gómez y el nombre del departamento al que pertenece el jefe.
SELECT CONCAT_WS(' ', je.nombre, je.ape1, je.ape2) AS nombre_completo_jefe, d.nombre AS nombre_departamento
FROM empleado e
JOIN empleado je ON e.jefe_id = je.id
JOIN departamento d ON je.departamento_id = d.id
WHERE e.nombre = 'Elena' and e.ape1='Pérez' and e.ape2 = 'Gómez';

-- Consulta para obtener el nombre del departamento y el numero de ventas realizadas por 
-- los empleados que trabajan en ese departamento.
SELECT d.nombre AS nombre_departamento, COUNT(v.id) AS total_ventas
FROM departamento d
JOIN empleado e ON d.id = e.departamento_id
JOIN venta v ON e.id = v.empleado_id
GROUP BY d.nombre;
